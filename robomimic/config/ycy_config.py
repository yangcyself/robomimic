...
class YCYConfig(BaseConfig):
    ...
    def algo_config(self):
        ...
        self.algo.optim_params.encoder.learning_rate.initial = 1e-4      # policy learning rate
        self.algo.optim_params.encoder.learning_rate.decay_factor = 1e-2  # factor to decay LR by (if epoch schedule non-empty)
        self.algo.optim_params.encoder.learning_rate.epoch_schedule = [] # epochs where LR decay occurs
        self.algo.optim_params.encoder.regularization.L2 = 0.00          # L2 regularization strength
        self.algo.optim_params.decoder.learning_rate.initial = 1e-4      # policy learning rate
        self.algo.optim_params.decoder.learning_rate.decay_factor = 1e-2  # factor to decay LR by (if epoch schedule non-empty)
        self.algo.optim_params.decoder.learning_rate.epoch_schedule = [] # epochs where LR decay occurs
        self.algo.optim_params.decoder.regularization.L2 = 0.00          # L2 regularization strength

        # loss weights
        self.algo.loss.kl_weight = 10.0      # L2 loss weight
        self.algo.latent_dim = 32
        self.algo.max_len = 500
